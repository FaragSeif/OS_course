...
page_t* find_page(page_t *pages, int id, size_t frames_count)
{
...
page_t* available_frame(page_t *pages, size_t frames_count)
{
...
int main(int argc, char *argv[])
{
	...
	int frame_count = strtol(argv[1], NULL, 10);

	page_t *pages = calloc(frame_count, sizeof(page_t));		// page table
	if(pages == NULL)
	{
	...
	while(fscanf(input, "%d", &page_id) == 1)
	{
		...
		page_t *page = find_page(pages, page_id, frame_count);
		...
		else				// No. it is a miss
		{
			...
			page = available_frame(pages, frame_count);	// find a place for that page
